...
The input data needs to be placed in input folder.
Following are the commands that can be used to run a makefile.

make-alone : used to run on local machine
make-cloud : used to run on cloud

The make file needs to be configured according to the programming environment.
Following are the fields that need to be changed in make file

scala.root : stores the path of hadoop distribution
...
b) stderr.txt: Contains syslog from AWS-EMR
